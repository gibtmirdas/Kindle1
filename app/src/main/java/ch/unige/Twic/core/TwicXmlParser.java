...
import ch.unige.Twic.language.CodeName;
import ch.unige.Twic.language.CodeNamesMap;
import ch.unige.Twic.language.LanguagePair;
...
public class TwicXmlParser implements TwicFields{
    ...
    public static void parseLanguageList(String response){
        ...
        PairsList.add(new LanguagePair(AUTO, AUTO));
        for(int i = 0; i < pairs.getLength(); i++){
            ...
            if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (!PairsList.containsKey(src))
                    PairsList.add(new LanguagePair(src, AUTO));
                LanguagePair srcAuto = new LanguagePair(AUTO, tgt);
                if(!PairsList.containsCouple(srcAuto))
                    PairsList.add(srcAuto);
                PairsList.add(new LanguagePair(src, tgt));
            }
        }
        ...
        for(int i = 0; i < codeNames.getLength(); i++){
            ...
            CodeNamesMap.put(code, new CodeName(code, name));
        }
        CodeNamesMap.put(AUTO, new CodeName(AUTO, AUTO));
    }
